...
import option.all._
import testkit.Test
import option.parser._
import Helpers._

class OptionParserTest extends Test {
  describe("Options parser") {
    it("parse string option with name") {
      val (_, res) = parseOption(string --"host").run(Seq("--host", "localhost"))
    ...
    it("parse string option with both name and abbreviation using abbreviation") {
      val (_, res) = parseOption(string --"host" -'h').run(Seq("-h", "localhost"))
      res.success should === (Some("localhost"))
    }

    it("parse string option with both name and abbreviation using name") {
      val (_, res) = parseOption(string --"host" -'h').run(Seq("--host", "localhost"))
      res.success should === (Some("localhost"))
    }
    ...
    it("parse string option with default") {
      val (_, res) = parseOption(string --"host" -'h'-~ or("myhost")).run(Seq())
    ...
    it("parse int option with default") {
      val (_, res) = parseOption(int --"port" -'p' -~ or(5432)).run(Seq("-p", "8080"))
      res.success should === (8080)
    }

    it("fail to parse int option") {
      val (_, res) = parseOption(int --"port" -'p' -~ or(5432)).run(Seq("-p", "8080s"))
    ...
    it("parse required option") {
      val (_, res) = parseOption(string --"debug" -'d' -~ req).run(Seq("--debug", "true"))
      res.success should === ("true")
    }
    ...
    it("fail to parse required option") {
      val (_, res) = parseOption(int --"debug" -'d').run(Seq("--debug", "value"))
      res.failure.toList.collect {
        case c: FailedToDecodeOption => c.getClass.getName
      }.distinct.size should === (1)
